...
function config = phantom_config_eTM_examp_001(config)

    % Kinetic variables to be reported from model...
    config.VARIABLES     = {'ktrans','ve','vp'};
    ...
    % values...
    config.PARAM_IN_FILE = '.\config\param_values_eTM_examp_001.csv';
    ...
    config.KEP           = config_read_parameter_values(config.KIN_VARS{4}, config.PARAM_IN_FILE, config);

    % Scale factors for output maps [ktrans, ve, vp, kep]...
    config.MAP_SF        = [1000.0, 1000.0, 10000.0, 1000.0];
